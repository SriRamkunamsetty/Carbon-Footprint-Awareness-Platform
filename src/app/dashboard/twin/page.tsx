...
export default function CarbonTwinPage() {
  ...
  const baseCarKm = 600;
  const baseAC = 120; // 4 hrs/day * 30 days
  const baseMeatMeals = 30; // beef/poultry meals per month

  // Simulation parameters
  const [transitDays, setTransitDays] = useState(0); // days swapping car for transit (saves 20km per swap day)
  const [acReduction, setAcReduction] = useState(0); // hours reduced per day
  const [vegMealsSwaps, setVegMealsSwaps] = useState(0); // beef meals swapped for vegetables per week
  const [renewableUtility, setRenewableUtility] = useState(0); // renewable percentage offset (0-100%)
  ...
  const acPowerKw = 1.5;
  const gridFactor = 0.47;
  const beefServingFactor = 6.5;
  const vegServingFactor = 0.3;

  // 1. Calculate Baseline Carbon
  const baselineCarCarbon = baseCarKm * 0.21;
  const baselineACCarbon = baseAC * acPowerKw * gridFactor;
  const baselineMeatCarbon = baseMeatMeals * beefServingFactor;
  const baselineOther = 80; // shopping, waste, water constants
  const baselineTotal = Math.round(baselineCarCarbon + baselineACCarbon + baselineMeatCarbon + baselineOther);

  // 2. Calculate Projected Carbon
  // Car substitution: each transit day swaps 20km/week (80km/month) of driving with transit
  const monthlyCarKmSwapped = Math.min(baseCarKm, transitDays * 80);
  const projectedCarKm = baseCarKm - monthlyCarKmSwapped;
  const projectedCarCarbon = projectedCarKm * carFactor + monthlyCarKmSwapped * transitFactor;

  // AC reduction: reduce hours * 30 days
  const projectedACHours = Math.max(0, baseAC - acReduction * 30);
  // Renewable multiplier applies to AC
  const electricityMultiplier = 1 - renewableUtility / 100;
  const projectedACCarbon = projectedACHours * acPowerKw * gridFactor * electricityMultiplier;

  // Meat reduction: swaps vegMealsSwaps per week (4 * vegMealsSwaps per month)
  const monthlyMeatSwaps = Math.min(baseMeatMeals, vegMealsSwaps * 4);
  const projectedMeatMeals = baseMeatMeals - monthlyMeatSwaps;
  const projectedDietCarbon = projectedMeatMeals * beefServingFactor + monthlyMeatSwaps * vegServingFactor;

  // Aggregate Projected Total
  const projectedTotal = Math.round(projectedCarCarbon + projectedACCarbon + projectedDietCarbon + (baselineOther * electricityMultiplier));

  // 3. Offsets and Savings Calculations
  const carbonSaved = Math.max(0, baselineTotal - projectedTotal);
  const moneySaved = Math.round((monthlyCarKmSwapped * 0.15) + (acReduction * 30 * acPowerKw * 0.16)); // gasoline + electricity utility rates
  const treesEquivalent = Math.round(carbonSaved * 12 * 0.045); // average tree absorbs 22kg CO2/year (~1.83kg/month)
    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-xs text-zinc-500 mt-0.5">
          Simulate carbon-reduction scenarios. Adjust your lifestyle toggles to visualize projections in real-time.
        </p>
      </div>
          ...
          <GlassCard className="p-6 space-y-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-[10px] text-zinc-500 leading-normal">
                Saves approx 20 km of driving per day. Replaces high-impact combustion with green transit.
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              />
              <p className="text-[10px] text-zinc-500 leading-normal">
                Trims runtime on heavy climate compressors, lowering household utility grids.
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              />
              <p className="text-[10px] text-zinc-500 leading-normal">
                Replaces high-emission red meat servings with low-impact plant proteins.
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <input
                ...
                max="100"
                step="10"
                value={renewableUtility}
                onChange={(e) => setRenewableUtility(Number(e.target.value))}
                className="w-full h-1 bg-zinc-800 rounded-lg appearance-none cursor-pointer accent-emerald-500"
              />
              <p className="text-[10px] text-zinc-500 leading-normal">
                Uses clean solar/wind credits to reduce the carbon intensity of all electric usage.
              </p>
            </div>
